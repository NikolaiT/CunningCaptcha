...
abstract class CunningCanvas {
    ...
    protected final function merge($dy = 0, $dx = 0, $array2d = array()) {
        /* If it fits I sits */
        ...
        for ($t = 1; $t < 200; $t++) {
            ...
            while (True) {
                ...
                if ($d["x_der"] != 0 && $d["y_der"] != 0) {
                    ...
                    if ($coord == "x")
                        if (($s["x_sum"] < 0) ? $s["x_sum"] > -$APPROX_EPSILON : $s["x_sum"] < $APPROX_EPSILON)
                            return $tn;
                    if ($coord == "y")
                        if (($s["y_sum"] < 0) ? $s["y_sum"] > -$APPROX_EPSILON : $s["y_sum"] < $APPROX_EPSILON)
                            return $tn;
...
class CunningGlyph extends CunningCanvas {
    ...
    private function _fill() {
        ...
        for ($i = 0; $i < $this->get_height(); $i++) {
            for ($j = 0; $j < $this->get_width(); $j++) {
                $ctl_line = array(new CunningPoint(0, $i), new CunningPoint($j, $i));
                foreach ($this->glyphdata as $shapetype) {
                    foreach ($shapetype as $shape) {
                        if ($this->_intersects($ctl_line, $shape)) {
                            $num_intersections++;
                        }
                    }
                }
                if ($num_intersections == 1) {
                    //printf("Going to set pixel at (x|y)= $j, $i in RED! <br/>");
                    $this->set_pixel($j, $i, $color = CunningCanvas::SIGNAL_RED);
                }
                $num_intersections = 0;
            }
        }
     ...
     * @return boolean
     */
    private function _lines_intersect($line1, $line2) {
        ...
        $m1 = ($line1[1]->y - $line1[0]->y) / ($dx1);
        $m2 = ($line2[1]->y - $line2[0]->y) / ($dx2);
        $n1 = ($line1[1]->x * $line1[0]->y - $line1[0]->x * $line1[1]->y) / ($dx1);
        $n2 = ($line2[1]->x * $line2[0]->y - $line2[0]->x * $line2[1]->y) / ($dx2);
        ...
        if (($x > max($line1[0]->x, $line1[1]->x) || $x > max($line2[0]->x, $line2[1]->x)) || # Found x is bigger than a maximal x-coordinate of a at least one line
                ($x < min($line1[0]->x, $line1[1]->x) || $x < min($line2[0]->x, $line2[1]->x)))     # Found x is smaller than a minimal x-coordinate of a at least one line
            return False;

        //return Point(int(x), int(m1*x + n1));
        return True; // reaching here mans that we found a intersection point.
    }
     ...
     * @return boolean
     */
    private function _line_bezier_intersects($line, $bezier) {
    ...
    private function _intersects($line, $shape) {
        if (count($shape) == 2) /* Handle line-line intersections */ {
            return $this->_lines_intersect($line, $shape);
        }

        if (count($shape) == 4) { /* Handle cubic splines */
            //return $this->_line_bezier_intersects($line, $shape);
            return False;
        }
    }
...
class CunningCaptcha extends CunningCanvas {
    ...
    /* All glyphs */
    const ALPHABET = 'y, W, G, a, H, i, f, b, n, S, X, k, E, Q, e, k2';
            ...
            case 'ppm':
                ...
                for ($i = 0; $i < $this->height; $i++) {
                    for ($j = 0; $j < $this->width; $j++) {
                        if (isset($bm[$i][$j])) {
                            if ($bm[$i][$j] == CunningCanvas::SIGNAL_RED) {
                                fwrite($h, '255 0 0' . "\t");
                            } else
                                fwrite($h, '0 0 0' . "\t");
                        } else {
